...
class SmartMotorVehicleScraper(MotorVehicleComplianceDocumentScraper):
    ...
    def smart_download_pipeline(self, link_data, page_context=''):
        ...
        logger.info("✅ Stage 2.5 passed: No blacklist terms")
        ...
        logger.info("✅ Stage 3 passed: Metadata relevant")
        ...
        if self.config['enable_bert']:
            ...
            logger.info("⏭️  Stage 4 skipped: BERT disabled")

        # STAGE 5: Full Download
        logger.info("🎉 ALL CHECKS PASSED - Downloading full PDF...")
        ...
        if local_path:
            ...
            logger.warning("⚠️  Download failed")
    ...
    def _scrape_single_page(self, site_name, url):
        ...
        try:
            ...
            if pdf_links and pdf_links[0].get('is_irdai_document', False):
                logger.info("🔍 Processing IRDAI document-detail links...")
        ...
        try:
            for i, doc_link in enumerate(document_links, 1):
                ...
                if not self.navigate_to_page(doc_link['url']):
                    logger.warning("⚠️  Failed to load document page")
                    continue
                ...
                if not pdf_links:
                    logger.warning("⚠️  No PDF links found on document page")
                    continue
    ...
    def _print_progress_stats(self):
        ...
        logger.info(f"\n{'='*80}")
        logger.info("📊 PROGRESS STATISTICS")
        logger.info(f"{'='*80}")
